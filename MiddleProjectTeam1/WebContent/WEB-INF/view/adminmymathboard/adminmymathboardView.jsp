...
<div class="comment-form" id="commentForm">
    <h3>답변 작성</h3>
    <form action="<%=request.getContextPath()%>/adminMymathBoardAddComment.do" method="get">
...
$(document).on('click', '.editCommentBtn', function() {
    ...
    if (newContent) {
        $.ajax({
            url: '<%=request.getContextPath()%>/adminMymathBoardEditComment.do',
            type: 'get',
            data: {
...
$(document).on('click', '.deleteCommentBtn', function() {
    ...
    if (confirm("정말 이 댓글을 삭제하시겠습니까?")) {
        $.ajax({
            url: '<%=request.getContextPath()%>/adminMymathBoardDeleteComment.do',
            type: 'get',
            data: {
                comment_no: commentNo
            },
