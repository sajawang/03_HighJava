...
<div class="container">
    ...
    <!-- 문의 등록 버튼 -->
    <a id="openRegisterModal" href="javascript:void(0)">나의수학해결소 등록</a>
    
    <!-- 문의 모달 -->
    <div id="InquiriesModal" class="modal">
        <div class="modal-content">
            <span class="close">&times;</span>
            <h2 id="modalTitle">나의수학해결소 작성하기</h2>
            <form id="InquiriesForm" method="get" action="<%=request.getContextPath()%>/adminMymathBoardUpdate.do" enctype="multipart/form-data">
...
%>
            ...
            <tr>
                ...
                <td><a href="<%=request.getContextPath()%>/adminMymathBoardView.do?post_no=<%=pvo.getPost_no()%>"><%=pvo.getPost_title() %></a></td>
...
%>
    <tr>
        <td colspan="10">나의수학해결소 목록이 없습니다.</td>
    </tr>
<%
}
%>
...
document.addEventListener("DOMContentLoaded", function() {
    ...
    openRegisterBtn.addEventListener('click', function() {
        ...
        modalTitle.innerText = '나의수학해결소 작성하기';
        submitBtn.innerText = '작성하기';
        
        // 등록 모드로 전환 ''을 이용해서 빈칸으로 만들기
        InquiriesForm.action = '<%=request.getContextPath()%>/adminMymathBoardInsert.do';
    ...
    editBtns.forEach(function(btn) {
        btn.addEventListener('click', function() {
            modal.style.display = 'block';
            modalTitle.innerText = '나의수학해결소 수정하기';
            ...
            // 수정할 때의 action 설정
            InquiriesForm.action = '<%=request.getContextPath()%>/adminMymathBoardUpdate.do';
        });
    });
...
document.querySelectorAll('.deleteBtn').forEach(function(btn) { 
    btn.addEventListener('click', function() {
        if (confirm("진짜 삭제하시겠습니까?")) {
            ...
            $.ajax({
                type: "POST",
                url: "<%=request.getContextPath()%>/adminMymathBoardDelete.do",  // 삭제 처리할 URL
                data: { post_no: post_no },
                success: function(response) {
                    // 성공적으로 삭제된 후 리스트를 다시 로드하거나 페이지를 리다이렉트
                    alert("삭제되었습니다.");
                    location.href = "<%=request.getContextPath()%>/adminMymathBoardList.do";  // 리스트 페이지로 리다이렉트
                },
                error: function() {
