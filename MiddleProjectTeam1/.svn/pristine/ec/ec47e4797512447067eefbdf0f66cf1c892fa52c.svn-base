package kr.or.ddit.dao;

import java.util.List;
import java.util.Map;

import kr.or.ddit.vo.CustomerVO;

public interface IAdmTeacherDao {
	
	/**
	 * memId를 매개변수로 받아서 해당 선생님의 정보를 가져오는 메서드
	 * 
	 * @param memId 회원Id
	 * @return UserVO 선생님정보를 가지고 있는 UserVO객체
	 */
	public CustomerVO detailTeacher(String memId);
	
	/**
	 * UserVO에 저장된 자료를 DB에 insert하는 메서드
	 * 
	 * @param CustomerVO DB에 insert할 자료가 저장된 UserVO객체
	 * @return 작업 성공 : 1, 작업 실패 : 0
	 */
	public int insertTeacher(CustomerVO vo);
	
	/**
	 * UserVO에 저장된 자료를 DB에 update하는 메서드
	 * 
	 * @param CustomerVO DB에 update할 자료가 저장된 UserVO객체
	 * @return 작업 성공 : 1, 작업 실패 : 0
	 */
	public int updateTeacher(CustomerVO vo);
	
	/**
	 * memId를 매개변수로 받아 해당 선생님의 정보를  DB에서 delete하는 메서드
	 * 
	 * @param memId 회원Id
	 * @return 작업 성공 : 1, 작업 실패 : 0
	 */
	public int deleteTeacher(String memId);
	
	// 페이징 처리 
	/**
	 * Map객체(페이징처리변수)를 매개변수로 받아서 해당 게시글의 정보를 가져오는 메서드
	 * 
	 * @param Map<String, Object> 페이징처리변수
	 * @return List<PostVO> PostVO객체가 저장된 List객체
	 */
	public List<CustomerVO> getAllTeacher(Map<String, Object> map);
	
	/**
	 * Map객체(페이징처리변수)를 매개변수로 받아서 해당 게시글의 개수를 가져오는 메서드
	 * 
	 * @param Map<String, Object> 페이징처리변수
	 * @return count 게시글의 개수를 가지고있는 count
	 */
	public int countTeacher(Map<String, Object> map);
}
