...
// 	})
	  function openPopup(){
	        window.open("<%=request.getContextPath()%>/WebContent/view/idInput.jsp", "new", "toolbar=no, menubar=no, scrollbars=yes, resizable=no, width=700, height=700, left=0, top=0" );
	    }

	$('#idFind').on('click',function(){
		
// 		$.ajax({
<%-- 			url:'<%=request.getContextPath()%>/popUp.do', --%>
...
<form  class="login-form" action="<%=request.getContextPath() %>/AdminLogin.do"  method="post">
